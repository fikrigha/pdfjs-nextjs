...
const PDFViewerApplication = {
  ...
  async initialize(appConfig) {
    ...
    // pd: Add this to PDFViewerApplication initialization code
    if (!window.localStorage.pdfAnnotations) {
      window.localStorage.pdfAnnotations = JSON.stringify({});
    }
...
class AnnotationStorageHandler {
  ...
  async saveAnnotations() {
    try {
      // Get the serialized annotation data
      const annotationData = await this.serializeAnnotations();
      const storageKey = `pdfAnnotations-${this.pdfDocument.fingerprints[0]}`;

      // Convert annotation data to serializable format
      const serializableData = {};
      for (const [key, value] of Object.entries(annotationData)) {
        serializableData[key] = this.#makeSerializable(value);
      }
  ...
  async loadAnnotations() {
    try {
      ...
      // Convert back to the format annotation storage expects
      const annotationData = {};
      for (const [key, value] of Object.entries(parsedData)) {
        annotationData[key] = this.#makeDeserializable(value);
      }

      await this.deserializeAnnotations(annotationData);
      return true;
    } catch (error) {
  ...
  async serializeAnnotations() {
    const annotations = {};

    // Get all serializable data from annotation storage
    for (const [key, value] of this.annotationStorage.serializable) {
  ...
  async deserializeAnnotations(annotationData) {
    for (const [key, value] of Object.entries(annotationData)) {
      this.annotationStorage.setValue(key, value);
    }
  }

  // Helper method to make data serializable
  ...
  // Helper method to convert serialized data back to original format
  #makeDeserializable(value) {
    if (value && typeof value === "object") {
      if (value._type === "Map") {
        return new Map(
          value.value.map(([k, v]) => [k, this.#makeDeserializable(v)])
        );
      } else if (Array.isArray(value)) {
        return value.map((item) => this.#makeDeserializable(item));
      } else {
        const result = {};
        for (const [k, v] of Object.entries(value)) {
          result[k] = this.#makeDeserializable(v);
        }
        return result;
      }
    }
  ...
  // Debug helper
  async logCurrentAnnotations() {
    const annotations = await this.serializeAnnotations();
